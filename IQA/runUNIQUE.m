...
% Script will run UNIQUE for all images within a folder and save
% the values to csv
...
proc_subfolder = '/Real_Test/CodecError-5/processed/';
...
fprintf('| Starting UNIQUE analysis\n')
...
for ii = 1 : num_imgs
    ...
    % run UNIQUE
    unique(ii) = mslUNIQUE(gt_img,proc_img);
    fprintf('| Finished image %d \n', ii);
end

% Write UNIQUE values to csv
writematrix(unique,append(proc_path,'UNIQUE.csv'));
