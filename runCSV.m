...
% Script will run CSV for all images within a folder and save
% the values to csv
...
%% Run CSV
% Call csv which returns the perceived quality.

fprintf('| Starting CSV analysis\n')
csv_vals = zeros(num_imgs,1);
for ii = 1 : num_imgs
    % load images
    ...
    % run CSV
    csv_vals(ii) = csv(gt_img,proc_img);
    fprintf('| Finished image %d \n', ii);
end

% Write CSV values to csv
writematrix(csv_vals,append(proc_path,'CSV.csv'));
